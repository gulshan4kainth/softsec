...
try:
    ...
    raise SystemExit(f"Request to /api/rmap-get-link failed at network layer: {e}")
if r2.status_code != 200:
    print(f"[!] /api/rmap-get-link HTTP {r2.status_code}")
    ...
    try:
        ...
        body = r2.text[:500]
    raise SystemExit(f"Endpoint returned error: {body}")
resp2 = r2.json()
if "result" not in resp2:
    raise SystemExit(f"Server returned error in message2: {resp2}")
link_token = resp2["result"]
print("[+] Link token (secret):", link_token, "reused=" + str(resp2.get('reused'))) 
